...
SIMPLEFSDP_OPTS=${SIMPLEFSDP_OPTS:-""}
SIMPLEFSDP_CHORUS_OPTS=${SIMPLEFSDP_CHORUS_OPTS:-"${SIMPLEFSDP_PREFETCH_OPTS:-"--simplefsdp_enable_chorus"}"}
...
MODEL_PATH=${MODEL_PATH:-"${HOME}/.cache/modelscope/hub/models/LLM-Research/"}
# MODEL_PATH=${MODEL_PATH:-"${HOME}/.cache/modelscope/hub/models/baichuan-inc/"}
# MODEL_PATH=${MODEL_PATH:-"${HOME}/.cache/modelscope/hub/models/Qwen/"}
...
for BATCH_SIZE in ${BATCH_SIZE_OPTS[@]}; do
    for SEQ_LENGTH in ${SEQ_LENGTH_OPTS[@]}; do
        ...
        # bash ./run_multinode.sh --backend simplefsdp ${ARGS} ${COMPILE_OPTS} ${SIMPLEFSDP_FAST_OPTS} ${SIMPLEFSDP_OPTS}
        bash ./run_multinode.sh --backend simplefsdp ${ARGS} ${COMPILE_OPTS} ${SIMPLEFSDP_FAST_OPTS} ${SIMPLEFSDP_CHORUS_OPTS}
